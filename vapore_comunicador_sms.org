...
* Ultimas Mejoras al programa antes de produccion
...
** Modificar memoria bkp cuando este disponible placa/micro

* Hardware nueva ver 1.0
...
* Hardware ver 2.0
** DONE Gpio pins change
   CLOSED: [2022-04-12 Mon 11:40]


* Firmware ver 1.3
** DONE retraso la salida del sms si esta verificando la red
   CLOSED: [2021-11-30 Tue 09:15]
** DONE modifico todo para que en hard 1.0 hard 1.1 se pueda utilizar firm 1.3 o 1.2
   CLOSED: [2021-11-30 Tue 09:15]

* Firmware ver 1.4
** envia el estado de bateria
   - si ve un mensaje BATERIA:
     envia el valor en volts al numero que pide el reporte

** se puede configurar reportes de bateria
*** con mensajes
    - REPORTAR_BAT:1 habilita reportes del estado de bateria
    - REPORTAR_BAT:0 deshabilita

*** manda por bateria baja y bateria desconectada
    - no envia cuando se restituye el valor de bateria

*** para que pueda enviar reportes
    - habiliar los reportes
    - cargar el numero a reportar

* Firmware ver 1.5
** DONE corrije el reboot del modulo cuando contesta mal
   CLOSED: [2022-05-23 Mon 10:32]
   - igual funcionalidad que el firmware 1.4 pero corrije 
     errores de comunicacion con el modulo

   - cuando aparecen errores de comunicacion entre gsm y micro
     reinicia

** Errores permitidos
   - MAX_STARTUP_ERRORS = 10, cuando esta arrancando el modulo 10 errores en 
     comandos AT

   - MAX_COMMS_ERRORS = 20, cuando esta funcionando y chequea RSSI + estado
     de la red

   - MAX_SMS_ERRORS = 20, cuando intenta enviar un reporte por SMS

* Firmware ver 1.6
** DONE Puede conectar con Server por GPRS
   CLOSED: [2022-05-23 Mon 10:11]
   - agrego nuevos comandos SMS
     IPNO:
     IP:configuracion de ipv4
     IPDN:configuracion con nombre de dominio

** Mensajes tipicos
   - IPNO:
     configura solo uso por SMS
     
   - IP:192.168.1.1,PROTO:TCP,PORT:3000,APN:gprs.movistar.com
     configura automaticamente uso por GPRS, luego SMS
     ip, protocolo, puerto, apn del proveedor

   - IPDN:vhfkirno.duckdns.com,PROTO:TCP,PORT:3000,APN:gprs.movistar.com
     configura automaticamente uso por GPRS, luego SMS
     nombre de dominio, protocolo, puerto, apn del proveedor
